...
## Current state
**R16 Stage A / R16.1–R16.4 is implemented and automated-green in `main`; the current gate is Mandatory Studio Gate A — HUMAN STUDIO PENDING. R16.5 motor/grip/mass tuning must not begin until that live Studio gate passes. Historical B17/G0 remains the downstream HUMAN_GATE after R16 completes.**

The current Stage-A contract keeps X/Y translation physical, mechanically locks Z translation to the lane, and keeps the BodyCollider upright about world X/Y/Z while only the legs intentionally rotate. `PhysicsConfig.LegGeometry` owns the canonical hub offsets. R16.3A recenters the cleaned authoritative shape on its bounds midpoint by translation only, preserving size/proportions; one centered ShapeSpec is duplicated to both legs. R16.4 retains the 180° twin-leg phase and redraw phase-preservation contract.
...
R14.1–R14.10 closed the pre-G0 runtime/tooling gaps: authoritative shape parity, Studio G0 presentation harness, fail-closed gate runner, Default collision contract, bounded network pending/failure handling, shape-preserving G0 recovery, atomic rollback regression, validation copy, CI Rojo build, and shared StrokeTypes/RemoteNames ownership. Final code/tooling evidence head `8a6a05a31427346d2a1437820ffa759f21fca90c`, run `34387618626`: **120 passed, 0 failed** plus successful **Rojo build**. R14.11 reconciled repository status/evidence documents to that code head without promoting the human gate.

R15.1 replaced the failed finite-force lane follower with the current mechanical `PlaneConstraint` Z lock. Its historical free-world-Z body rotation detail is superseded by R16.1. R16 Stage A then added the upright-body, canonical-hub, centered-authoritative-shape and twin-leg phase contracts. Current authoritative status/evidence belongs to `docs/SESSION.md`; automated/CI/Rojo evidence never substitutes for the pending Studio solver check.
...
A01–A04 and B01–B02 remain recorded ACCEPTED. B03–B16 implementation and regression specs exist in `main`, but required Studio/human evidence remains pending. **Mandatory R16 Studio Gate A is the current hard stop; B17/G0 remains a later hard stop before C01/M0.5.**
...
- **R16.3/R16.3A — one drawing → two centered authoritative legs:** server cleanup recenters the shape by bounds translation only; size is not normalized away; accepted preview and both physical legs consume that authoritative centered geometry.
...
Decision records include `docs/DECISION_LOG_CORE_AUDIT_REPAIR_2026-09-09.md`, `docs/DECISION_LOG_PRE_G0_REVIEW_R07_R09_2026-09-09.md`, `docs/DECISION_LOG_PRE_G0_BUG_SWEEP_R10_R12_2026-09-09.md`, `docs/DECISION_LOG_PRE_G0_RUNTIME_CLOSURE_R14_2026-09-09.md`, and the current R16 reference-parity design/decision material in `docs/`.
...
Deterministic clamp/dedupe, DrawInputRect semantic coordinates, RDP simplification, open-polyline resampling, length/bounds validation and malformed/non-finite coverage. R16.3A server authority then translates the cleaned shape bounds midpoint to `(0,0)` without resizing before physical mapping.
...
`LegShapeService` validates semantic stroke intent and owns ShapeSpec/version progression. Exact semantic remotes are `SubmitStroke` and `StrokeResult`; accepted results carry the server-authoritative centered points used by accepted preview. Client data cannot author world geometry, CFrame, segment plans, ShapeVersion or rewards.
...
## R16 Stage A local Studio gate
Default Studio mode remains `G0`, but the current local task is **R16 Mandatory Studio Gate A**, not B17 acceptance. After current `main` is synced, a Play session should run the synchronous B03–B16 specs through the R14.3 gate runner and then leave one interactive human G0 racer only if the gate reaches READY.

Expected Stage-A local evidence includes:
...
- normal `laneDeviation <= 0.03`, with unexplained `>0.08` a failure;
- normal body angular deviation `<=1°`; strong-contact disturbance `<=3°` and recovery to `<=1°` within `0.25 s`;
- body remains physically free to move/rise/fall in X/Y rather than being position-scripted;
- configured hubs remain symmetric/fixed and do not make the body continuously scrape flat solely because of axle placement;
- one accepted drawing creates exactly two matching physical legs;
- same-size copies drawn at different DrawInputRect positions recenter to the same authoritative accepted/physical geometry, while genuinely different sizes remain different sizes;
- left/right gait is consistent with the 180° phase contract and redraw does not visibly reset phase;
- observer Roblox Character remains non-participating in racer physics.

Only after Stage A passes may R16.5 begin. After the ordered R16 Stage B/C gates and R16.11 docs freeze, empirical B17/G0 still follows `docs/55_EMPIRICAL_PRODUCT_GATE_PROTOCOL.md`: six unique external testers and all fixed PASS criteria. Do not mark either gate PASS from CI alone.

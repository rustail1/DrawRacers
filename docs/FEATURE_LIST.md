# FEATURE LIST — SCOPE SOURCE OF TRUTH v1.4.3
...
**Current implementation integrity:** bounded CORE/pre-G0 repair **R01–R12**, runtime/evidence closure **R14.1–R14.11**, planar correction **R15/R15.1**, R16 Stage A **R16.1–R16.4**, Stage B **R16.5–R16.7**, Stage C **R16.8–R16.10**, and current **R16.3B stroke-origin/reference-parity correction** are present in `main` at repository level. Live Roblox solver/visual evidence remains **HUMAN STUDIO PENDING**.  
**Current R16 contract:** X/Y translation remains physical, Z translation remains mechanically lane-locked, and body rotation about world X/Y/Z is upright constrained; specifically **rotation about world Z: locked/corrected**. R16.2 owns hub offsets in `PhysicsConfig.LegGeometry`. **R16.3B supersedes R16.3A bounds-center semantics:** after cleanup the **first cleaned point** becomes authoritative origin `(0,0)` by translation only, one ShapeSpec is duplicated to both legs, the visible semantic DrawInputRect is wide `1.75:1` but isotropic by height, physical collider Parts stay hidden, and presentation geometry is nonphysical. R16.4 retains the 180° twin-leg phase contract. Unified Studio mode `R16FINAL` runs the R16 evidence path before human G0 handoff.  
**Historical evidence retained:** pre-R06 **66 passed, 0 failed**; R09 repair **80 passed, 0 failed**; final R10–R12 code head `e2bedd34696bb99da43878c19d7984c9134c8bef` / run `34363706915` verified **88 passed, 0 failed**. Final R14 code/tooling head `8a6a05a31427346d2a1437820ffa759f21fca90c` / run `34387618626` verified **120 passed, 0 failed** plus successful **Rojo build**. The first R15 force-based implementation passed automation but failed real Studio with `laneDeviation 5.199`. R15.1 mechanical-plane production head `be44304bf00391e05c7d7750609730a7368ebc87` / run `34394991926` verified **125 passed, 0 failed** plus successful **Rojo build**. Stage-A repository head `81c7d84c542160f07fc4fe986df89e5aa69f8f73` / run `34448666472` verified **142 passed, 0 failed**. Stage-B implementation head `2197882e4c641e7c1d17a6dfc538f23fd1a521e6` / run `34451426807` verified **145 passed, 0 failed** plus Rokit and **Rojo build PASS**. Final pre-R16.3B R16.10 code head `3838a994f5b5164b64f3cdee934e5bc84f7be7a4` / run `34454820796` verified **149 passed, 0 failed**, Rokit install PASS and **Rojo build PASS**.  
**Current gate:** **Studio Gate A — HUMAN STUDIO PENDING; Studio Gate B — HUMAN STUDIO PENDING; Studio Gate C — HUMAN STUDIO PENDING.** Historical B17/G0 remains the downstream **HUMAN_GATE** after R16 Studio evidence; implementation/CI/Rojo build alone do not promote B03–B16 to ACCEPTED.  
...
- **ACTIVE / R16 Stage A / R16.1–R16.4 IMPLEMENTED; Studio Gate A — HUMAN STUDIO PENDING** — upright body, canonical hubs, authoritative two-leg geometry and 180° twin-leg phase. R16.3B supersedes only the earlier bounds-center input-origin detail.
- **ACTIVE / R16 Stage B / R16.5–R16.7 IMPLEMENTED; Studio Gate B — HUMAN STUDIO PENDING** — single-owner leg material config and ROUND flat measurement, solver-owned Y evidence on Steps/Gap, full canonical reference-shape registry and comparative matrix.
- **ACTIVE / R16 Stage C / R16.8–R16.10 IMPLEMENTED; Studio Gate C — HUMAN STUDIO PENDING** — 10 moving redraws, reference-side presentation, observer hiding, Wall evidence, and aggregate canonical evidence harness.
- **ACTIVE / R16.3B REFERENCE-PARITY CORRECTION; HUMAN STUDIO PENDING** — wide semantic input surface; first-point authoritative origin; local sequence-scoped presentation anchor; hidden physical collider boxes; nonphysical visual leg rendering; F3-hidden debug panel; unified synchronous `R16FINAL` Studio handoff.
...
- **R01** — one `GeometryMath` plan owner + authoritative ShapeSpec→LegAssembly path. Current R16.3B uses a **wide semantic drawing surface** and height-based isotropic normalization; the older square-surface wording is superseded.
...
- **R07** — strict B12 outer payload validation/rate work-ordering; complete B16 raw/physics/motor telemetry and DEV/STAGING gating; semantic surface ownership tightened. R16.3B supersedes its historical square-input detail.
...
- **R16.3 / R16.3B Pivot + First-point Origin** — one accepted stroke becomes one ShapeSpec and exactly two same-XY legs. The server translates the **first cleaned point** to `(0,0)` without resizing, mirroring or rotating; `StrokeResult.acceptedPoints` and physical geometry use that same first-point-relative authoritative shape. The previous R16.3A bounds-center/bounds midpoint mechanical-origin rule is superseded.
...
- Historical Stage-A repository evidence: head `81c7d84c542160f07fc4fe986df89e5aa69f8f73`, run `34448666472`, **142 passed, 0 failed**, successful Rokit install and **Rojo build**.
- **Mandatory Studio Gate A — HUMAN STUDIO PENDING.** Later implementation authorization never retroactively marks this gate PASS.

### R16 Stage B — reference feel/evidence implementation
- **R16.5 Motor / Grip / Mass — IMPLEMENTED, Studio Gate B — HUMAN STUDIO PENDING** — `PhysicsConfig.PhysicalMaterials.LegSegment` owns the existing leg material values; Stage-B evidence measures ROUND flat speed after `2 s` settle over `3 s`, requiring `4–7 studs/s`, motors enabled and no anti-stall. No production motor/torque/grip numbers are blindly changed to force a target.
- **R16.6 Vertical Physics — IMPLEMENTED, Studio Gate B — HUMAN STUDIO PENDING** — normal locomotion remains solver-owned in Y; harness measures HOOK rise on SmallSteps and SMALL_ROUND fall in GapSmall. No scripted Y locomotion is added.
- **R16.7 Reference Shape Matrix — IMPLEMENTED, Studio Gate B — HUMAN STUDIO PENDING** — executable `ROUND_01`, `LONG_BAR_01`, `SMALL_ROUND_01`, `HOOK_01`, `ASYM_01`, `SUBOPTIMAL_01` run from reset through exact comparative rules for flat/steps/gap/tunnel, including `SuboptimalWorseRatio=0.20` and `noUniversalWinner`.
- Historical Stage-B repository evidence: head `2197882e4c641e7c1d17a6dfc538f23fd1a521e6`, run `34451426807`, **145 passed, 0 failed**, successful Rokit install and **Rojo build PASS**.
...
### R16.3B — stroke-origin / visual reference parity correction
- **ACTIVE / repository contract reconciliation** — `RawSemanticHalfWidth=1.75`, `RawSemanticHalfHeight=1.0`; one wide visible/capture DrawInputRect, isotropic normalization by height.
- **First-point authority** — after cleanup the first cleaned point becomes `(0,0)` and is the mechanical hub origin; the bounds midpoint is not required to be zero. This supersedes R16.3A bounds-center origin semantics.
- **Network unchanged** — `SubmitStroke` remains `{sequence, points}`; server returns first-point-relative authoritative `acceptedPoints`; a local sequence-scoped presentation anchor never becomes server authority.
- **Presentation/physics split** — physical collider boxes stay hidden and authoritative; visible leg segments/joints are nonphysical (`CanCollide/CanTouch/CanQuery=false`, massless).

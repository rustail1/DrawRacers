...
function C01CoreV3SharedAxleSpec.run()
	...
	local ok, failure = xpcall(function()
		...
		assert(leftLocal.Y < -0.5, "C01 LeftMount must sit below body center so valid legs can bear load")
		assert(rightLocal.Y < -0.5, "C01 RightMount must sit below body center so valid legs can bear load")

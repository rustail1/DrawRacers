...
def test_r16_3b_history_is_superseded_by_cr3_free_draw_support_anchor() -> None:
    ...
    cr3 = read("docs/superpowers/specs/2026-09-14-core-repair-v3-free-draw-single-phase-design.md")
    current = read("docs/CR3_CURRENT_SOURCE_OF_TRUTH.md")
    assert "R16.3B" in decision
    assert "fixed pivot" in cr2.lower()
    assert "free draw" in cr3.lower()
    assert "support anchor" in cr3.lower()
    assert "CR3" in current
    assert "historical" in current.lower()


def test_r16_3b_shared_math_keeps_wide_isotropic_surface_and_translates_to_geometry_support() -> None:
...
def test_r16_3b_authoritative_shape_uses_support_anchor_without_network_schema_expansion() -> None:
    ...
    assert "network payload" in current.lower()


def test_r16_3b_draw_ui_uses_full_wide_free_draw_surface_without_center_marker() -> None:
...
def test_r16_3b_current_shape_and_motor_tuning_remains_bounded_pending_g0() -> None:
    config = read("src/shared/Config/PhysicsConfig.lua")
...
def test_r16_3b_graphite_presentation_survives_cr3() -> None:
    drawing = read("src/client/Controllers/DrawingController.lua")
    leg = read("src/server/Runtime/LegAssembly.lua")

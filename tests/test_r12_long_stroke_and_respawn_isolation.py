...
def test_r12_long_semantic_stroke_compacts_instead_of_freezing_at_cap() -> None:
    ...
    assert "function DrawingController:_compactRawSemanticPoints" in drawing
    compact_start = drawing.index("function DrawingController:_compactRawSemanticPoints")
    compact_end = drawing.index("function DrawingController:_tryAppendRawSemanticPoint", compact_start)
    compact_block = drawing[compact_start:compact_end]
    assert "compactEveryOther(self._rawSemanticPoints)" in compact_block

    append_start = drawing.index("function DrawingController:_tryAppendRawSemanticPoint")
    append_end = drawing.index("function DrawingController:_appendLivePoint", append_start)
    append_block = drawing[append_start:append_end]
    assert "self:_compactRawSemanticPoints()" in append_block
    assert "if #samples >= PhysicsConfig.StrokeProcessing.MaxRawPoints then\n\t\treturn\n\tend" not in append_block
    assert "forceFinal or" in append_block
